...
import type { WebsiteService, WebsitePortfolio, ServiceDetail } from "../types/index.js";

const router = Router();

const fallbackServices: WebsiteService[] = [
...
const fallbackPortfolio: WebsitePortfolio[] = [
...
const fallbackServiceDetails: ServiceDetail[] = [
...
router.get("/services", async (_req, res) => {
  if (!supabase) return res.json(fallbackServices);
  const { data, error } = await supabase.from("website_services").select("*");
  if (error) return res.status(500).json({ error: error.message });
  return res.json(data);
});

router.get("/portfolio", async (_req, res) => {
  if (!supabase) return res.json(fallbackPortfolio);
  const { data, error } = await supabase.from("website_portfolio").select("*");
  if (error) return res.status(500).json({ error: error.message });
  return res.json(data);
});

router.get("/service-details", async (_req, res) => {
  if (!supabase) return res.json(fallbackServiceDetails);
  const { data, error } = await supabase.from("website_service_details").select("*");
  if (error) return res.status(500).json({ error: error.message });
  return res.json(data);
});
