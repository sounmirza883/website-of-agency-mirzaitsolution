...
router.get("/tasks", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
  if (!supabase) return res.json([]);
  let query = supabase.from("employee_tasks").select(TASK_WITH_NAMES).order("id", { ascending: false }).limit(500);
  if (req.query.employeeId) query = query.eq("employee_id", Number(req.query.employeeId));
  if (req.query.projectId) query = query.eq("project_id", Number(req.query.projectId));
...
router.post("/tasks", requireAuth, requireRole("admin"), asyncHandler(async (req: AuthedRequest, res) => {
  ...
  if (!employeeId || !task) return res.status(400).json({ error: "employeeId and task are required" });
  if (!supabase) return res.status(503).json({ error: "Database not configured" });
  ...
  if (projectId && !projectName) {
    const { data: p } = await supabase.from("admin_projects").select("name").eq("id", Number(projectId)).maybeSingle();
...
router.patch("/tasks/:id", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
  const id = Number(req.params.id);
  if (!supabase) return res.status(503).json({ error: "Database not configured" });
  ...
  if (employeeId !== undefined) patch.employee_id = employeeId ? Number(employeeId) : null;
  if (projectId !== undefined) patch.project_id = projectId ? Number(projectId) : null;
  ...
  if (estimatedMinutes !== undefined) patch.estimated_minutes = estimatedMinutes ? Number(estimatedMinutes) : null;
  if (clientVisible !== undefined) patch.client_visible = !!clientVisible;
  if (Object.keys(patch).length === 0) return res.status(400).json({ error: "No fields to update" });

  const { data, error } = await supabase.from("employee_tasks").update(patch).eq("id", id).select(TASK_COLUMNS).maybeSingle();
  if (error) return res.status(500).json({ error: error.message });
  if (!data) return res.status(404).json({ error: "Task not found" });
  if (patch.progress !== undefined || patch.status !== undefined) await rollUpProjectProgress(data.projectId as number | null);
  return res.json(data);
}));

router.delete("/tasks/:id", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
  if (!supabase) return res.status(503).json({ error: "Database not configured" });
  const { data: existing } = await supabase.from("employee_tasks").select("project_id").eq("id", Number(req.params.id)).maybeSingle();
  const { error } = await supabase.from("employee_tasks").delete().eq("id", Number(req.params.id));
  if (error) return res.status(500).json({ error: error.message });
...
router.get("/time-entries", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
  ...
  let query = supabase
    ...
    .limit(1000);
  if (req.query.employeeId) query = query.eq("employee_id", Number(req.query.employeeId));
  if (req.query.from) query = query.gte("started_at", String(req.query.from));
  if (req.query.to) query = query.lte("started_at", String(req.query.to));
  const { data, error } = await query;
  if (error) return res.status(500).json({ error: error.message });
  return res.json(data);
}));

router.get("/task-reports", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
  if (!supabase) return res.json([]);
  let query = supabase
    .from("task_reports")
...
router.get("/daily-reports", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
  if (!supabase) return res.json([]);
  let query = supabase
    .from("daily_reports")
