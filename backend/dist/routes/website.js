...
router.post("/contact", async (req, res) => {
    ...
    const first = await supabase_js_1.supabase.from("website_contact_submissions").insert(withBudget).select().single();
    if (!first.error)
    ...
    if (/budget|currency|schema cache|does not exist/i.test(first.error.message)) {
        ...
        const retry = await supabase_js_1.supabase.from("website_contact_submissions").insert(base).select().single();
        if (retry.error)
