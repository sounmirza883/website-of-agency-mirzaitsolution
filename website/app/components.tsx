...
export function ContactForm() { const [sent, setSent] = useState(false); return <form className="contact-form" onSubmit={(event) => { event.preventDefault(); setSent(true); }}><h3>Send Us a <strong>Message</strong></h3>{["Full Name", "Email Address", "Phone Number"].map((label) => <div className="form-group" key={label}><input required type={label === "Email Address" ? "email" : "text"} placeholder=" " /><label>{label}</label></div>)}<div className="form-group"><select defaultValue=""><option value="">Select a service</option>{serviceLinks.map((item) => <option key={item}>{item}</option>)}</select></div><div className="form-group"><textarea required placeholder=" " /><label>Project Details</label></div><button type="submit" className="pill pill-primary"><Icon name="fa-paper-plane" /> {sent ? "Message Sent" : "Send Message"}</button>{sent && <p className="form-success">Thank you. We will get back to you shortly.</p>}</form>; }
