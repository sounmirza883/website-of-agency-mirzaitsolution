...
/**
 * Every project thread in one place. Admin previously had to open a modal per
 * project from the board, which gave no way to see where something new had
 * arrived. Same two-pane shape as the staff chat and the employee project chat.
 */
export default function AdminChatPage() {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-1">Project Chat</h1>
        <p className="text-sm text-gray-500">Every project conversation, in one place</p>
      </div>
      ...
      <div className="flex-1 flex gap-4 min-h-0">
          ...
          {(conversations ?? []).map((c: any) => (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                {c.unread > 0 && <span className="shrink-0 text-[11px] font-medium bg-accent text-gray-50 rounded-full px-1.5 py-0.5">{c.unread}</span>}
              </div>
              <p className="text-xs text-gray-500 truncate mt-0.5">{c.lastMessage ?? "No messages yet"}</p>
            </button>
          ))}
          {(conversations ?? []).length === 0 && <div className="px-4 py-8 text-center text-sm text-gray-400">No projects yet</div>}
        </aside>
        ...
        <section className="flex-1 flex flex-col bg-white border border-gray-200 rounded-xl min-w-0">
          ...
          <div className="flex-1 overflow-auto p-4 space-y-3">
            {(messages ?? []).map((m: any) => {
              ...
              return (
                <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[70%] rounded-xl px-3 py-2 ${mine ? "bg-accent text-gray-50" : "bg-gray-100 text-gray-900"}`}>
                    {!mine && <p className="text-[11px] font-medium text-gray-500 mb-0.5 capitalize">{m.senderRole}</p>}
            ...
            {activeId && (messages ?? []).length === 0 && <p className="text-center text-sm text-gray-400 py-8">No messages in this thread yet</p>}
            <div ref={bottomRef} />
          </div>
          ...
          <form onSubmit={handleSend} className="p-3 border-t border-gray-200 flex gap-2">
            ...
            />
            <button type="submit" disabled={!activeId || !text.trim() || sendMessage.isPending} className="bg-accent text-gray-50 text-sm font-medium px-4 py-2 rounded-lg disabled:opacity-50">
