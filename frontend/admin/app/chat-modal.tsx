    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-lg bg-white rounded-xl p-6 flex flex-col max-h-[80vh]">
        ...
        <div className="flex-1 overflow-auto space-y-2 mb-4 pr-1">
          ...
          {messages?.map((m: any) => (
            <div key={m.id} className={`flex ${m.senderRole === "admin" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[75%] rounded-lg px-3 py-2 text-sm ${m.senderRole === "admin" ? "bg-accent text-gray-50" : "bg-gray-100 text-gray-800"}`}>
        ...
        <form onSubmit={handleSend} className="flex gap-2">
          ...
          <button type="submit" disabled={sendMessage.isPending} className="bg-accent text-gray-50 text-sm font-medium px-4 py-2 rounded-lg disabled:opacity-50">Send</button>
        </form>
      </div>
    </div>
  );
