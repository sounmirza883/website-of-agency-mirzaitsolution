...
export function Footer() {
  return <footer><div className="container"><div className="footer-grid">
    <div className="footer-brand"><Link href="/" className="logo">Mirza IT <strong>Solution</strong></Link><p>Mirza IT Solution is a creative digital studio offering professional graphic design, video editing, motion graphics, UI/UX design, development, marketing, automation, and hosting services.</p><div className="footer-social"><a href="#" aria-label="Facebook"><i className="fab fa-facebook-f" /></a><a href="#" aria-label="Instagram"><i className="fab fa-instagram" /></a><a href="#" aria-label="YouTube"><i className="fab fa-youtube" /></a><a href="https://wa.me/92XXXXXXXXXX" aria-label="WhatsApp"><i className="fab fa-whatsapp" /></a></div></div>
...
export function FAQ() { const [open, setOpen] = useState(0); const questions = [["What services does Mirza IT Solution provide?", "Mirza IT Solution provides graphic design, video editing, motion graphics, UI/UX design, development, and marketing services."], ["Do you work with international clients?", "Yes, Mirza IT Solution works with clients worldwide through email, video calls, and project management tools."], ["Can you create a complete brand package?", "Yes, we can create logos, brand identity, social media designs, websites, videos, and marketing content."], ["Do you provide revisions?", "Yes, revisions are provided based on the project package and client requirements."], ["How can I start a project?", "Contact us through the form, email, or WhatsApp button to discuss your project."]]; return <div className="faq-list">{questions.map(([question, answer], index) => <div className={`faq-item ${open === index ? "active" : ""}`} key={question}><button className="faq-question" onClick={() => setOpen(open === index ? -1 : index)}>{question}<i className="fas fa-chevron-down" /></button>{open === index && <div className="faq-answer">{answer}</div>}</div>)}</div>; }
