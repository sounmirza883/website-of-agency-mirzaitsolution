...
export function Footer() {
  return <footer><div className="container"><div className="footer-grid">
    ...
    <div><h4>Contact</h4><ul><li><a href="mailto:your-email@example.com"><Icon name="fa-envelope" /> Email us</a></li><li><a href="https://wa.me/923292745959"><Icon name="fa-whatsapp" /> WhatsApp</a></li><li><Icon name="fa-globe" /> Available Worldwide</li></ul></div>
...
type PortfolioItem = { slug: string; title: string; category: string; filterKey: string; icon: string; description: string; image: string };

export function PortfolioFilter({ items }: { items: readonly PortfolioItem[] }) {
  ...
  return <><div className="portfolio-filters">{filters.map(([id, label]) => <button key={id} className={`filter-btn ${filter === id ? "active" : ""}`} onClick={() => setFilter(id)}>{label}</button>)}</div><div className="grid-3">{items.filter((item) => filter === "all" || item.filterKey === filter).map((item) => <Link href={`/portfolio/${item.slug}`} className="portfolio-card" key={item.slug}><div className="thumb"><Image src={item.image} alt={item.title} fill sizes="(max-width: 768px) 100vw, 33vw" style={{ objectFit: "cover" }} /></div><div className="info"><h4>{item.title}</h4><span>{item.category}</span><p>{item.description}</p></div></Link>)}</div></>;
}
