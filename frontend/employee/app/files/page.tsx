  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setOpen(true)} className="bg-emerald-600 text-white text-sm font-medium px-4 py-2 rounded-lg">+ Upload File</button>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>{files?.map((f) => <tr key={f.id} className="border-t border-gray-100 hover:bg-gray-50"><td className="px-5 py-3 font-medium">{f.name}</td><td className="px-5 py-3 text-gray-600">{f.project}</td><td className="px-5 py-3 text-gray-600">{f.size}</td><td className="px-5 py-3 text-gray-500">{f.uploaded}</td><td className="px-5 py-3"><span className={`text-xs font-medium px-2 py-0.5 rounded ${f.status === "Approved" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>{f.status}</span></td><td className="px-5 py-3">{f.url ? <a href={f.url} target="_blank" rel="noreferrer" className="text-emerald-600 font-medium">Download</a> : <span className="text-gray-300">Unavailable</span>}</td></tr>)}</tbody>
        </table>
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={() => setOpen(false)}>
          <form onSubmit={handleSubmit} onClick={(e) => e.stopPropagation()} className="w-full max-w-md bg-white rounded-xl p-6">
            ...
            <div className="flex justify-end gap-2 mt-6">
              ...
              <button type="submit" disabled={uploadFile.isPending} className="bg-emerald-600 text-white text-sm font-medium px-4 py-2 rounded-lg disabled:opacity-50">{uploadFile.isPending ? "Uploading…" : "Upload"}</button>
            </div>
          </form>
        </div>
      )}
