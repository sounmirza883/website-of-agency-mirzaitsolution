    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex bg-gray-100 rounded-lg p-1">
          <button onClick={() => setView("board")} className={`text-sm font-medium px-3 py-1.5 rounded-md ${view === "board" ? "bg-white shadow text-gray-900" : "text-gray-500"}`}>Board</button>
          <button onClick={() => setView("table")} className={`text-sm font-medium px-3 py-1.5 rounded-md ${view === "table" ? "bg-white shadow text-gray-900" : "text-gray-500"}`}>Table</button>
        </div>
      </div>
