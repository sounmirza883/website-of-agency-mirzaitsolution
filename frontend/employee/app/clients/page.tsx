  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setOpen(true)} className="bg-accent-2 text-gray-50 text-sm font-medium px-4 py-2 rounded-lg">+ Add Client</button>
      </div>
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={() => setOpen(false)}>
          <form onSubmit={handleSubmit} onClick={(e) => e.stopPropagation()} className="w-full max-w-md bg-white rounded-xl p-6">
            ...
            <div className="flex justify-end gap-2 mt-6">
              ...
              <button type="submit" disabled={createClient.isPending} className="bg-accent-2 text-gray-50 text-sm font-medium px-4 py-2 rounded-lg disabled:opacity-50">{createClient.isPending ? "Creating…" : "Create"}</button>
            </div>
          </form>
        </div>
      )}
